...
const SingleSelection = ({ interview }: Props) => {
  ...
  }, [questionIndex, interview.questions]);
  ...
  const { mutate: endInterview } = useMutation({
    mutationFn: async () => {
      const payload: z.infer<typeof endInterviewSchema> = {
        interviewId: interview.id
      };
      const response = await axios.post(`/api/endInterview`, payload);
      return response.data;
    }
  });
  ...
  if (hasEnded) {
    redirect(`/dashboard/result/${interview.id}`);
  }
      ...
      <div className="flex flex-row justify-between">
        <div className="flex flex-col">
          ...
          <div className="flex self-start mt-3 text-slate-400">
            ...
            {formatTimeDelta(differenceInSeconds(now, interview.timeStarted))}
          </div>
        </div>
